...
var groupings = [
	...
	},
	{
		...
		groups: [
			...
			{
				str: "Domestic (includes Fiat Chrysler)",
				x: 605,
				c: green,
				fn: R.propEq("Locale", "Traditional Domestic")
			}
		]
	},
	// { str: 'Top Seller',
		...
		groups: [
			{
				...
				y: 506,
				c: red,
				fn: R.propEq("Corp.", "Ford Motor Company")
			},
			{
				str: "GM",
				x: 59,
				y: 251,
				c: blue,
				fn: R.propEq("Corp.", "General Motors Corp.")
			},
			{
				str: "Toyota",
				x: 425,
				y: 380,
				c: green,
				fn: R.propEq("Corp.", "Toyota Motor Sales U.S.A. Inc.")
			},
			{
				str: "Fiat Chrysler",
				x: 306,
			...
			},
			{
				...
				y: 58,
				c: orange,
				fn: R.propEq("Corp.", "Nissan North America Inc.")
			},
			{
				str: "Honda",
				x: 284,
				y: 49,
				c: purple,
				fn: R.propEq("Corp.", "American Honda Motor Co. Inc.")
			},
			{
				str: "Hyundai/Kia",
				x: 688,
				...
				fn: function (d) {
					return (
						d["Corp."] == "Hyundai Motor America" ||
						d["Corp."] == "Kia Motors America Inc."
					);
				}
			},
			{
...
function ttMove(d) {
	...
	tt.style(
		"left",
		Math.min(Math.max(0, x - nBB.width / 2), window.innerWidth - nBB.width) +
			"px"
	);
}
...
function searchStr(str) {
	return data.filter(function (d) {
		var valuesStr = ["model", "Brand", "Type", "Locale"]
			.map(function (vStr) {
				return d[vStr].toLowerCase();
			})
			.join(" ");
...
function getCarType() {
	...
	var outTakes = data
		.filter(function (d) {
			return ~"Bentley, Rolls Royce, Maserati, Lamborghini, Porsche, and Ferrari".indexOf(
				d.Brand
			);
		})
		.map(ƒ("model"))
	...
	var outTakes = modelStr.map(function (d) {
		return { model: d, pos: "r" };
	});
	var str = JSON.stringify(outTakes, null, 2);
	console.log(str);
	...
	d3.select("svg")
		...
		.on("mousemove", function () {
			...
			d3.selectAll("text").attr("x", pos[0]).attr("y", pos[1]);
			console.log(pos);
		});
}
